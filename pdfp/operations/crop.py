...
class Converter(QObject):
    ...
    def convert(self, file_tree: QObject, pdf: str) -> None:
        """
        ...
        if not pathlib.Path(briss_location).exists():
            logger.error("Briss location invalid. Configure in settings.")
            return None
        try:
        ...
        if automation_enabled:
            ...
            logger.info("Cropping %s...", pdf)
            QApplication.processEvents()
            output_file = construct_filename(pdf, "crop_ps")
            try:
                process = subprocess.Popen(
                    ["java", "-jar", briss_location, "-s", pdf, "-d", output_file], stdout=subprocess.PIPE
                )
                progress = 0
                for _stdout_line in iter(process.stdout.readline, ""):
