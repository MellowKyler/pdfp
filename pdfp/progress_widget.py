...
class WorkerProgress(QWidget):
    ...
    def __init__(self, worker_name, width) -> None:
        super().__init__()
...
class ProgressWidget(QScrollArea):
    ...
    def __init__(self) -> None:
        ...
        self.pb_list.setAlignment(Qt.AlignmentFlag.AlignTop)
    ...
    def init_worker_progress(self, worker_name: str) -> None:
        ...
        logger.debug("Initializing new worker: %s", worker_name)
    ...
    def worker_done(self, worker_name) -> None:
        """
        Removes a given worker from the ProgressWidget and marks for deletion.
        Args:
            worker_name (str): Name of the worker.
        """
        logger.debug("Worker done: %s", worker_name)
    ...
    def revise_worker_label(self, worker_name, label_prefix) -> None:
        """
        ...
        logger.debug("Revising worker label: %s, %s", worker_name, label_prefix)
        if worker_name not in self.workers:
            self.worker_progress(worker_name, 0)
        _operation, file_path = worker_name.split("_", 1)
        filename = pathlib.Path(file_path).name
        worker = self.workers[worker_name]
